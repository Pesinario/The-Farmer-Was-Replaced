    ...
    def pruned_branches_no_dupes(history):
        def find_branches(list_of_positions):
            ...
            for pos in list_of_positions:
                if pos in nodes:
                    # We either backtracked or
                    # we are about to split into different branches.
                    current_branch.append(pos)
                    work_branches.append(prune_current(current_branch))
                    # we prune for the scenario that we backtracked.
...
#    branch_to_depth = find_branch_depth()
    ...
    def get_to_ancestor_smarter(start_index, treasure_index):
        ...
        last_common_ancestor = find_last_common_ancestor(
            my_ancestors, treasure_ancestors)
        ...
        quick_print("We are at most common ancestor",
                    (get_pos_x(), get_pos_y()))
